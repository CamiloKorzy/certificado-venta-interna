...
try:
    ...
    cur = conn.cursor()
    cur.execute("SELECT fecha, documento, comprobante, cliente, productonombre, equiposolicitantenombre, total FROM ceesa_cee_certificados_ventas_internas LIMIT 5")
    rows = cur.fetchall()
    for r in rows:
        print(r)
    cur.close()
